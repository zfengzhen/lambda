# tests/data/sync/test_iv.py
"""data.sync.iv 单元测试：IV 计算。"""
import math
import os
import logging
...
from data.sync.iv import bs_implied_vol, compute_ticker_iv
...
# ── parse_occ_symbol（已移到 core.options，IV 测试仍用之） ─────
...
def _fetch_snapshot(ticker: str, symbol: str, api_key: str) -> dict | None:
    """调用 Massive Option Contract Snapshot API。"""
...
class TestSnapshotIvValidation:
    """调用 Massive Snapshot API，对比实时 IV 和 B-S 反算。

    运行方式: pytest tests/data/sync/test_iv.py -m online -v -s --log-cli-level=INFO
    """
    ...
    def test_single_contract_iv(self):
        ...
        from core.options import build_occ_symbol
        ...
        if spot is None:
            pytest.skip(f"无法获取 {self.TICKER} 价格")

        strike = round(spot)
        symbol = build_occ_symbol(expiry, float(strike), "C")

        snapshot = _fetch_snapshot(self.TICKER, symbol, self.api_key)
        if not snapshot:
            pytest.skip(f"Snapshot 无数据: {symbol}")
