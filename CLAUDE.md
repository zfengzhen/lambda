...
│
├── cron_run.sh              # 定时任务入口脚本（激活venv → run → deploy）
├── com.lambda.scheduled-notify.plist  # macOS launchd 定时任务配置
├── requirements.txt
├── .venv/                   # 虚拟环境（不提交）
└── docs/
    ├── api/                 # API 文档（Massive 等）
    ├── superpowers/         # 设计文档与实施计划
    └── *.md                 # 策略研究文档（entry-timing / strategy 等）
```
...
# ── 定时任务（macOS launchd） ──
# 安装：将 plist 链接到 LaunchAgents 并加载
